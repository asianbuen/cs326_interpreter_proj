...
public class Activation {
	...
	public Activation(Method thisCode, 
		...
		for (int i=0; i<thisCode.getSignature().size(); i++) {
			variables[adr] = operandStack.peek(sigsize-i);	// store parameters into activation variables array
		...
		for (int i=0; i<thisCode.getSignature().size(); i++) {
			operandStack.pop();	// pop the parameters from operand stack
		}

		if (!thisCode.isStatic()) {
			variables[0] = operandStack.pop(); // store object from operand stack into address 0 on values array
