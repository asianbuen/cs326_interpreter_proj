...
public class Execution {
    ...
    public void dup(int opCode, OperandStack operandStack) {
        switch (opCode) {
            case RuntimeConstants.opc_dup:
                operandStack.push(Value.makeValue(operandStack.peek(1)));
                break;
    ...
    public void two(int opCode, int from, int to, OperandStack operandStack) {
        ...
        switch (from) {
            case Value.s_integer:
                ...
                switch (to) {
                    case Value.s_byte:
                        operandStack.push(new IntegerValue((int) ((byte) iv)));
                        break;
                    case Value.s_char:
                        operandStack.push(new IntegerValue((int) ((char) iv)));
                        break;
                    case Value.s_short:
                        operandStack.push(new IntegerValue((int) ((short) iv)));
                        break;
                    case Value.s_double:
                        operandStack.push(new DoubleValue((double) iv));
                        break;
                    case Value.s_float:
                        operandStack.push(new FloatValue((float) iv));
                        break;
                    case Value.s_long:
                        operandStack.push(new LongValue((long) iv));
                        break;
                }
                break;
            case Value.s_reference:
                break; /* none */
            case Value.s_double:
                double dv = ((DoubleValue) e).getValue();
                switch (to) {
                    case Value.s_float:
                        operandStack.push(new FloatValue((float) dv));
                        break;
                    case Value.s_integer:
                        operandStack.push(new IntegerValue((int) dv));
                        break;
                    case Value.s_long:
                        operandStack.push(new LongValue((long) dv));
                        break;
                }
                break;
            case Value.s_float:
                ...
                switch (to) {
                    case Value.s_integer:
                        operandStack.push(new IntegerValue((int) fv));
                        break;
                    case Value.s_double:
                        operandStack.push(new DoubleValue((double) fv));
                        break;
                    case Value.s_long:
                        operandStack.push(new LongValue((long) fv));
                        break;
                }
                break;
            case Value.s_long:
                ...
                switch (to) {
                    case Value.s_double:
                        operandStack.push(new DoubleValue((double) lv));
                        break;
                    case Value.s_float:
                        operandStack.push(new FloatValue((float) lv));
                        break;
                    case Value.s_integer:
                        operandStack.push(new IntegerValue((int) lv));
                        break;
                }
                break;
            case Value.s_byte:
        ...
        if (et1 == Value.s_integer) {
            ...
            switch (opCode) {
                case RuntimeConstants.opc_iand:
                    operandStack.push(new IntegerValue(va1 & va2));
                    break;
            ...
            switch (opCode) {
                case RuntimeConstants.opc_land:
                    operandStack.push(new LongValue(va1 & va2));
                    break;
        ...
        if (RuntimeConstants.opc_if_icmpeq <= opCode && opCode <= RuntimeConstants.opc_if_icmple) {
            ...
            switch (opCode) {
                case RuntimeConstants.opc_if_icmpeq:
                    return (value1 == value2 ? labelAddress : pc + 1);

                case RuntimeConstants.opc_if_icmpne:
                    return (value1 != value2 ? labelAddress : pc + 1);
                case RuntimeConstants.opc_if_icmplt:
                    return (value2 < value1 ? labelAddress : pc + 1);
    ...
    private int ifBranch(int opCode, int labelAddress, OperandStack operandStack, int pc) {
        ...
        switch (opCode) {
            case RuntimeConstants.opc_ifeq:
                return (value == 0 ? labelAddress : pc + 1);
    ...
    private void shift(int opCode, int type, OperandStack operandStack) {
        ...
        switch (opCode) {
            case RuntimeConstants.opc_ishl:
                operandStack.push(new IntegerValue(((IntegerValue) va2).getValue() << (31 & va1.getValue())));
                break;
            ...
            case RuntimeConstants.opc_lshl:
                operandStack.push(new LongValue(((LongValue) va2).getValue() << (63 & va1.getValue())));
                break;
    ...
    // this method only handles return instructions with a return value
    public Value val_ret(int opCode, OperandStack operandStack) {
            switch(opCode) {
                    case RuntimeConstants.opc_areturn:
                        if (operandStack.peek(1).getType() != Value.s_reference) {
                            Error.error("type must be 'Reference'");
                        }
                        break;
                    case RuntimeConstants.opc_ireturn:
                        if (operandStack.peek(1).getType() != Value.s_integer) {
                            Error.error("type must be 'Integer'");
                        }
                        break;
                    case RuntimeConstants.opc_dreturn:
                        if (operandStack.peek(1).getType() != Value.s_double) {
                            Error.error("type must be 'Double'");
                        }
                        break;
                    case RuntimeConstants.opc_freturn:
                        if (operandStack.peek(1).getType() != Value.s_float) {
                            Error.error("type must be 'Float'");
                        }
                        break;
                    case RuntimeConstants.opc_lreturn:
                        if (operandStack.peek(1).getType() != Value.s_long) {
                            Error.error("type must be 'Long'");
                        }
                        break;
            }
            Value retVal = operandStack.peek(1); // obtain the value to return prior to updating activation variable
            pc = activation.getReturnAddress()+1; // pc will point to the next available inst. right after the calling inst.
            currentMethod = activation.getReturnCode();
            activationStack.pop();
            activation = activationStack.top(); // activate the activation variable
        ...
        while (!done) {
            ...
            switch (opCode) {
                ...
                case RuntimeConstants.opc_aconst_null:
                    operandStack.push(new ReferenceValue(null));
                    break;

                case RuntimeConstants.opc_bipush:
                    operandStack.push(new IntegerValue((byte) inst.getOperand()));
                    break;
                case RuntimeConstants.opc_sipush:
                    operandStack.push(new IntegerValue((short) inst.getOperand()));
                    break;
                case RuntimeConstants.opc_dconst_0:
                    operandStack.push(new DoubleValue((double) 0.0));
                    break;
                case RuntimeConstants.opc_dconst_1:
                    operandStack.push(new DoubleValue((double) 1.0));
                    break;
                case RuntimeConstants.opc_fconst_0:
                    operandStack.push(new FloatValue(0.0F));
                    break;
                case RuntimeConstants.opc_fconst_1:
                    operandStack.push(new FloatValue(1.0F));
                    break;
                case RuntimeConstants.opc_fconst_2:
                    operandStack.push(new FloatValue(2.0F));
                    break;
                ...
                case RuntimeConstants.opc_iconst_4:
                    operandStack.push(new IntegerValue(4));
                    break;
                case RuntimeConstants.opc_iconst_5:
                    operandStack.push(new IntegerValue(5));
                    break;
                case RuntimeConstants.opc_lconst_0:
                    operandStack.push(new LongValue(0L));
                    break;
                case RuntimeConstants.opc_lconst_1:
                    operandStack.push(new LongValue(1L));
                    break;
                case RuntimeConstants.opc_ldc:
                case RuntimeConstants.opc_ldc_w:
                case RuntimeConstants.opc_ldc2_w:
                    operandStack.push(((Ldc) inst).getValue());
                    break;

                // LOADING AND STORING
                case RuntimeConstants.opc_aload:
                    activation.load(Value.s_reference, inst.getOperand(), operandStack);
                    break;

                case RuntimeConstants.opc_aload_0:
                    activation.load(Value.s_reference, 0, operandStack);
                    break;
                case RuntimeConstants.opc_aload_1:
                    activation.load(Value.s_reference, 1, operandStack);
                    break;
                case RuntimeConstants.opc_aload_2:
                    activation.load(Value.s_reference, 2, operandStack);
                    break;
                case RuntimeConstants.opc_aload_3:
                    activation.load(Value.s_reference, 3, operandStack);
                    break;
                case RuntimeConstants.opc_astore:
                    activation.store(Value.s_reference, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_astore_0:
                    activation.store(Value.s_reference, 0, operandStack);
                    break;
                case RuntimeConstants.opc_astore_1:
                    activation.store(Value.s_reference, 1, operandStack);
                    break;
                case RuntimeConstants.opc_astore_2:
                    activation.store(Value.s_reference, 2, operandStack);
                    break;
                case RuntimeConstants.opc_astore_3:
                    activation.store(Value.s_reference, 3, operandStack);
                    break;

                case RuntimeConstants.opc_dload:
                    activation.load(Value.s_double, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_dload_0:
                    activation.load(Value.s_double, 0, operandStack);
                    break;
                case RuntimeConstants.opc_dload_1:
                    activation.load(Value.s_double, 1, operandStack);
                    break;
                case RuntimeConstants.opc_dload_2:
                    activation.load(Value.s_double, 2, operandStack);
                    break;
                case RuntimeConstants.opc_dload_3:
                    activation.load(Value.s_double, 3, operandStack);
                    break;
                case RuntimeConstants.opc_dstore:
                    activation.store(Value.s_double, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_dstore_0:
                    activation.store(Value.s_double, 0, operandStack);
                    break;
                case RuntimeConstants.opc_dstore_1:
                    activation.store(Value.s_double, 1, operandStack);
                    break;
                case RuntimeConstants.opc_dstore_2:
                    activation.store(Value.s_double, 2, operandStack);
                    break;
                case RuntimeConstants.opc_dstore_3:
                    activation.store(Value.s_double, 3, operandStack);
                    break;

                case RuntimeConstants.opc_fload:
                    activation.load(Value.s_float, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_fload_0:
                    activation.load(Value.s_float, 0, operandStack);
                    break;
                case RuntimeConstants.opc_fload_1:
                    activation.load(Value.s_float, 1, operandStack);
                    break;
                case RuntimeConstants.opc_fload_2:
                    activation.load(Value.s_float, 2, operandStack);
                    break;
                case RuntimeConstants.opc_fload_3:
                    activation.load(Value.s_float, 3, operandStack);
                    break;
                case RuntimeConstants.opc_fstore:
                    activation.store(Value.s_float, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_fstore_0:
                    activation.store(Value.s_float, 0, operandStack);
                    break;
                case RuntimeConstants.opc_fstore_1:
                    activation.store(Value.s_float, 1, operandStack);
                    break;
                case RuntimeConstants.opc_fstore_2:
                    activation.store(Value.s_float, 2, operandStack);
                    break;
                case RuntimeConstants.opc_fstore_3:
                    activation.store(Value.s_float, 3, operandStack);
                    break;

                case RuntimeConstants.opc_iload:
                    activation.load(Value.s_integer, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_iload_0:
                    activation.load(Value.s_integer, 0, operandStack);
                    break;
                case RuntimeConstants.opc_iload_1:
                    activation.load(Value.s_integer, 1, operandStack);
                    break;
                case RuntimeConstants.opc_iload_2:
                    activation.load(Value.s_integer, 2, operandStack);
                    break;
                case RuntimeConstants.opc_iload_3:
                    activation.load(Value.s_integer, 3, operandStack);
                    break;
                case RuntimeConstants.opc_istore:
                    activation.store(Value.s_integer, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_istore_0:
                    activation.store(Value.s_integer, 0, operandStack);
                    break;
                case RuntimeConstants.opc_istore_1:
                    activation.store(Value.s_integer, 1, operandStack);
                    break;
                case RuntimeConstants.opc_istore_2:
                    activation.store(Value.s_integer, 2, operandStack);
                    break;
                case RuntimeConstants.opc_istore_3:
                    activation.store(Value.s_integer, 3, operandStack);
                    break;

                case RuntimeConstants.opc_lload:
                    activation.load(Value.s_long, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_lload_0:
                    activation.load(Value.s_long, 0, operandStack);
                    break;
                case RuntimeConstants.opc_lload_1:
                    activation.load(Value.s_long, 1, operandStack);
                    break;
                case RuntimeConstants.opc_lload_2:
                    activation.load(Value.s_long, 2, operandStack);
                    break;
                case RuntimeConstants.opc_lload_3:
                    activation.load(Value.s_long, 3, operandStack);
                    break;
                case RuntimeConstants.opc_lstore:
                    activation.store(Value.s_long, inst.getOperand(), operandStack);
                    break;
                case RuntimeConstants.opc_lstore_0:
                    activation.store(Value.s_long, 0, operandStack);
                    break;
                case RuntimeConstants.opc_lstore_1:
                    activation.store(Value.s_long, 1, operandStack);
                    break;
                case RuntimeConstants.opc_lstore_2:
                    activation.store(Value.s_long, 2, operandStack);
                    break;
                case RuntimeConstants.opc_lstore_3:
                    activation.store(Value.s_long, 3, operandStack);
                    break;

                // DATA CONVERSION
                case RuntimeConstants.opc_d2f:
                    two(opCode, Value.s_double, Value.s_float, operandStack);
                    break;
                case RuntimeConstants.opc_d2i:
                    two(opCode, Value.s_double, Value.s_integer, operandStack);
                    break;
                case RuntimeConstants.opc_d2l:
                    two(opCode, Value.s_double, Value.s_long, operandStack);
                    break;
                case RuntimeConstants.opc_f2d:
                    two(opCode, Value.s_float, Value.s_double, operandStack);
                    break;
                case RuntimeConstants.opc_f2i:
                    two(opCode, Value.s_float, Value.s_integer, operandStack);
                    break;
                case RuntimeConstants.opc_f2l:
                    two(opCode, Value.s_float, Value.s_long, operandStack);
                    break;
                case RuntimeConstants.opc_i2b:
                    two(opCode, Value.s_integer, Value.s_byte, operandStack);
                    break;
                case RuntimeConstants.opc_i2c:
                    two(opCode, Value.s_integer, Value.s_char, operandStack);
                    break;
                case RuntimeConstants.opc_i2d:
                    two(opCode, Value.s_integer, Value.s_double, operandStack);
                    break;
                case RuntimeConstants.opc_i2f:
                    two(opCode, Value.s_integer, Value.s_float, operandStack);
                    break;
                case RuntimeConstants.opc_i2l:
                    two(opCode, Value.s_integer, Value.s_long, operandStack);
                    break;
                case RuntimeConstants.opc_i2s:
                    two(opCode, Value.s_integer, Value.s_short, operandStack);
                    break;
                case RuntimeConstants.opc_l2d:
                    two(opCode, Value.s_long, Value.s_double, operandStack);
                    break;
                case RuntimeConstants.opc_l2f:
                    two(opCode, Value.s_long, Value.s_float, operandStack);
                    break;
                case RuntimeConstants.opc_l2i:
                    two(opCode, Value.s_long, Value.s_integer, operandStack);
                    break;
                ...
                case RuntimeConstants.opc_ddiv:
                case RuntimeConstants.opc_drem:
                    binOp(opCode, Value.s_double, operandStack);
                    break;

                case RuntimeConstants.opc_fadd:
                    binOp(opCode, Value.s_float, operandStack);
                    break;
                case RuntimeConstants.opc_fsub:
                case RuntimeConstants.opc_fmul:
                case RuntimeConstants.opc_fdiv:
                ...
                case RuntimeConstants.opc_idiv:
                case RuntimeConstants.opc_irem:
                    binOp(opCode, Value.s_integer, operandStack);
                    break;
                ...
                case RuntimeConstants.opc_if_icmpgt:
                case RuntimeConstants.opc_if_icmple:
                    pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump) inst).getLabel()), operandStack, pc);
                    continue;
                case RuntimeConstants.opc_ifeq:
                case RuntimeConstants.opc_ifne:
                case RuntimeConstants.opc_iflt:
                ...
                case RuntimeConstants.opc_ifgt:
                case RuntimeConstants.opc_ifle:
                    pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump) inst).getLabel()), operandStack, pc);
                    continue;

                    // NEGATING
                case RuntimeConstants.opc_dneg:
                    negate(opCode, Value.s_double, operandStack);
                    break;
                case RuntimeConstants.opc_fneg:
                    negate(opCode, Value.s_float, operandStack);
                    break;
                case RuntimeConstants.opc_ineg:
                    negate(opCode, Value.s_integer, operandStack);
                    break;
                case RuntimeConstants.opc_lneg:
                    negate(opCode, Value.s_long, operandStack);
                    break;
                ...
                case RuntimeConstants.opc_ior:
                case RuntimeConstants.opc_ixor:
                    logic(opCode, Value.s_integer, operandStack);
                    break;
                case RuntimeConstants.opc_land:
                case RuntimeConstants.opc_lor:
                case RuntimeConstants.opc_lxor:
                    logic(opCode, Value.s_long, operandStack);
                    break;
                ...
                case RuntimeConstants.opc_ishr:
                case RuntimeConstants.opc_iushr:
                    shift(opCode, Value.s_integer, operandStack);
                    break;
                case RuntimeConstants.opc_lshl:
                case RuntimeConstants.opc_lshr:
                case RuntimeConstants.opc_lushr:
                    shift(opCode, Value.s_long, operandStack);
                    break;

                // SWAPPING
                case RuntimeConstants.opc_swap:
                    swap(opCode, operandStack);
                    break;
                ...
                case RuntimeConstants.opc_ifnull: {
                    ...
                    if (opCode == RuntimeConstants.opc_ifnonnull) {
                        if (((ReferenceValue) va).getValue() != null)
                            pc = currentMethod.getLabelAddress(((Jump) inst).getLabel());
                    } else if (((ReferenceValue) va).getValue() == null)
                        pc = currentMethod.getLabelAddress(((Jump) inst).getLabel());
                    continue; // ;-)
                }
                ...
                case RuntimeConstants.opc_checkcast:  // YOUR CODE HERE
                    Value v_cc = operandStack.peek(1); // checkcast does not push/pop anything to or from the stack
                    if (v_cc.getType() != Value.s_reference) {
                        Error.error("Value must be of type 'Reference'");
                    }

                    if (!Class.doesExtend(((ReferenceValue)v_cc).getValue().getThisClass(), ClassList.getClass(((ClassRef)inst).getClassName()))) {
                        Error.error("unable to cast --> class/superclass of object and parameter do not match");
                    }

                    // else, do nothing --> this indicates successful checkcast
                    break;

                    // FIELD RELATED
                case RuntimeConstants.opc_getfield: // YOUR CODE HERE
                    Value getField_obj = operandStack.pop();

                    if (getField_obj.getType() != Value.s_reference) {
                        Error.error("value must be of type 'Reference'");
                    }
                    // check if the class name does not match
                    else if (!((ReferenceValue)getField_obj).getValue().getThisClass().getClassName().equals(((FieldRef)inst).getClassName())) {
                        Error.error("class name does not match the class name of the object in the stack");
                    }
                    // check if field is static
                    else if (((ReferenceValue)getField_obj).getValue().getThisClass().getField(((FieldRef)inst).getFieldName()).isStatic()) {
                        Error.error("the field must be non-static");
                    }
                    // push value into stack
                    else {
                        operandStack.push(((ReferenceValue)getField_obj).getValue().getField((FieldRef)inst));
                    }
                    break;

                case RuntimeConstants.opc_getstatic: // YOUR CODE HERE
                    operandStack.push(ClassList.getClass(((FieldRef)inst).getClassName()).getStatic((FieldRef)inst));
                    break;

                case RuntimeConstants.opc_putfield: // YOUR CODE HERE
                    Value value_obj = operandStack.pop(); // V
                    Value putField_obj = operandStack.pop(); // O

                    // perform assertions/error checking
                    if (putField_obj.getType() != Value.s_reference) {
                        Error.error("value must be of type 'Reference'");
                    }
                    // check if the class name does not match
                    else if (!((ReferenceValue)putField_obj).getValue().getThisClass().getClassName().equals(((FieldRef)inst).getClassName())) {
                        Error.error("class name does not match the class name of the object in the stack");
                    }
                    // check if the field is static
                    else if (((ReferenceValue)putField_obj).getValue().getThisClass().getField(((FieldRef)inst).getFieldName()).isStatic()) {
                        Error.error("the field must be non-static");
                    }
                    else {
                        // error checking done, place the value into the field
                        ((ReferenceValue)putField_obj).getValue().putField((FieldRef)inst, value_obj);
                    }
                    break;

                case RuntimeConstants.opc_putstatic: // YOUR CODE HERE
                    Value putStatic_val = operandStack.pop();

                    // static and type checking already occurs in the putStatic() method
                    ClassList.getClass(((FieldRef)inst).getClassName()).putStatic(ClassList.getClass(((FieldRef)inst).getClassName()), (FieldRef)inst, putStatic_val);
                    break;


                    // INSTANCEOF
                case RuntimeConstants.opc_instanceof:// YOUR CODE HERE
                    Value v_iof = operandStack.pop();
                    if (v_iof.getType() != Value.s_reference) {
                        Error.error("value must be of type 'Reference'");
                    }
                    else {
                        if (((ReferenceValue)v_iof).getValue() == null) {
                            operandStack.push(new IntegerValue(0));
                        }
                        else if (((ReferenceValue)v_iof).getValue().getThisClass().getClassName().equals(((ClassRef)inst).getClassName())) {
                            operandStack.push(new IntegerValue(1));
                        }
                        else {
                            operandStack.push(new IntegerValue(0));
                ...
                case RuntimeConstants.opc_invokeinterface: // YOUR CODE HERE
                    Value iface_obj = operandStack.peek(((InterfaceInvocation)inst).getParamCount() + 1);
                    if (iface_obj.getType() != Value.s_reference) {
                        Error.error("value must be of type 'Reference'");
                    }

                    // checks if object implements/inherits class-name
                    if (Class.doesExtend(((ReferenceValue)iface_obj).getValue().getThisClass(), ClassList.getClass(((InterfaceInvocation)inst).getClassName()))){
                        Method iface_method = ((ReferenceValue)iface_obj).getValue().getThisClass().getVirtualMethod(((InterfaceInvocation)inst).getMethodName() + "/" + ((InterfaceInvocation)inst).getSignature().toString());

                        // check if method is static
                        if (iface_method.isStatic()) {
                            Error.error("method must be non-static");
                        }

                        // create new activation
                        Activation i_activation = new Activation(iface_method, pc, currentMethod, operandStack, ((ReferenceValue)iface_obj).getValue().getThisClass());
                        activationStack.push(i_activation);
                        currentMethod = iface_method;
                        pc = 0;
                        ...
                        Error.error("object does not implement/inherit 'class-name'");
                    }

                    continue;

                case RuntimeConstants.opc_invokestatic: // YOUR CODE HERE
                    if (!ClassList.getClass(((MethodInvocation)inst).getClassName()).getNonvirtualMethod(((MethodInvocation)inst).getMethodName() + "/" + ((MethodInvocation)inst).getSignature().toString()).isStatic()) {
                        Error.error("method must be static");
                    }

                    // check if the class-name is "Io"
                    else if (((MethodInvocation) inst).getClassName().equals("Io")) {
                        // call doIO()
                        doIO(in, (MethodInvocation)inst, operandStack);
                        break;
                    }

                    // check if the class-name is "java/lang/Object"
                    else if (((MethodInvocation) inst).getClassName().equals("java/lang/Object")) {
                        operandStack.pop();
                        break;
                    }

                    else {
                        Method st_method = ClassList.getClass(((MethodInvocation)inst).getClassName()).getNonvirtualMethod(((MethodInvocation)inst).getMethodName() + "/" + ((MethodInvocation)inst).getSignature().toString());

                        // create new activation
                        Activation st_activation = new Activation(st_method, pc, currentMethod, operandStack, ClassList.getClass(((MethodInvocation)inst).getClassName()));
                        activationStack.push(st_activation);
                        currentMethod = st_method;
                        pc = 0;
                        activation = st_activation;
                        operandStack = activation.getOperandStack();
                        // no need to update this_ because method is static
                    }
                    continue;

                case RuntimeConstants.opc_invokenonvirtual:
                    // obtain the method only through the ClassList, not the object on the stack...
                    Class nonV_cl = ClassList.getClass(((MethodInvocation)inst).getClassName());
                    Method nonV_method = nonV_cl.getNonvirtualMethod(((MethodInvocation)inst).getMethodName() + "/" + ((MethodInvocation)inst).getSignature().toString());

                    // check if method is static
                    if (nonV_method.isStatic()) {
                        Error.error("method must be non-static");
                    }

                    // check if the class-name is "Io"
                    if (((MethodInvocation) inst).getClassName().equals("Io")) {
                        // call doIO()
                        doIO(in, (MethodInvocation)inst, operandStack);
                        break;
                    }
                    ...
                    // check if the class-name is "java/lang/Object"
                    if (((MethodInvocation) inst).getClassName().equals("java/lang/Object")) {
                        operandStack.pop();
                        break;
                    }

                    // <--
                    // A quick HACK written very late at night ;-)
                    if (((MethodInvocation)inst).getClassName().equals("java/lang/StringBuilder")) {
                        if (((MethodInvocation)inst).getMethodName().equals("<init>")) {
                            System.out.println("INVOKESPECIAL: " + ((MethodInvocation)inst).getClassName());
                            operandStack.pop();
                            break;
                        } else if (((MethodInvocation)inst).getMethodName().equals("append")) {
                            Value v = operandStack.pop(); // A reference to the StringBuilder!
                            StringValue sv = (StringValue) operandStack.pop();
                            String str = sv.getValue() + v.toString();
                            operandStack.push(new StringValue(str));
                            break;
                        } else if (((MethodInvocation)inst).getMethodName().equals("toString"))
                            break;
                    }
                    // -->

                    /* 1. create a new activation
                            - this takes care of popping the parameters and the object
                            - and storing them into the variables array in Activation.java
                       2. push activation on the activation stack
                       3. set currentMethod to method being invoked and pc = 0
                       4. set this_ to object of method being invoked
                       5. update activation to the activation of method being invoked
                    */
                    Activation nonV_activation = new Activation(nonV_method, pc, currentMethod, operandStack, nonV_cl);
                    activationStack.push(nonV_activation);
                    currentMethod = nonV_method;
                    pc = 0;
                    activation = nonV_activation;
                    this_ = activation.getThis();
                    operandStack = activation.getOperandStack();
                    continue; // using continue here because pc is changed

                case RuntimeConstants.opc_invokevirtual: // YOUR CODE HERE
                    // obtain object on the stack by using peek(parameter size)
                    Value v_obj = operandStack.peek(ClassList.getClass(((MethodInvocation)inst).getClassName()).getVirtualMethod(((MethodInvocation)inst).getMethodName() + "/" + ((MethodInvocation)inst).getSignature().toString()).getSignature().size() + 1);
                    Method v_method = ((ReferenceValue)v_obj).getValue().getThisClass().getVirtualMethod(((MethodInvocation)inst).getMethodName() + "/" + ((MethodInvocation)inst).getSignature().toString());

                    // check if method is static
                    if (v_method.isStatic()) {
                        Error.error("method must be non-static");
                    }
                    ...
                    /* 1. create a new activation
                       ...
                       5. update activation to the activation of method being invoked
                    */
                    Activation v_activation = new Activation(v_method, pc, currentMethod, operandStack, ((ReferenceValue)v_obj).getValue().getThisClass());
                    activationStack.push(v_activation);
                    currentMethod = v_method;
                    pc = 0;
                ...
                case RuntimeConstants.opc_lreturn: // YOUR CODE HERE
                    if (currentMethod.getMethodName().equals("main")) {
                        done = true; // just terminate the execution loop...
                        continue;
                    }
                    ...
                    Value returnVal = val_ret(opCode, operandStack); // call return method and obtain return value
                    operandStack = activation.getOperandStack();
                    operandStack.push(returnVal); // push the returned value onto the current operand stack
                    continue;
                ...
                case RuntimeConstants.opc_new: // YOUR CODE HERE
                    ...
                    String nameofClass = ((ClassRef)inst).getClassName();
                    if (nameofClass.equals("java/lang/StringBuilder")) {
                        System.out.println("CLASSNAME: pushing an empty string");
                        operandStack.push(new StringValue("")); break;
                    }
                    // -->
                    else {
                        // Handle the stuff for the new on your own here ;-)
                        Object_ newObject = new Object_(ClassList.getClass(((ClassRef)inst).getClassName()));
                        operandStack.push(new ReferenceValue(newObject));
                        break;
                    }
