...
public class Execution {
	...
	if (RuntimeConstants.opc_if_icmpeq <= opCode && opCode <= RuntimeConstants.opc_if_icmple) {
		...
		case RuntimeConstants.opc_if_icmplt: return (value1 > value2 ? labelAddress : pc+1);
		case RuntimeConstants.opc_if_icmple: return (value1 >= value2 ? labelAddress : pc+1);
		case RuntimeConstants.opc_if_icmpgt: return (value1 < value2 ? labelAddress : pc+1);
		case RuntimeConstants.opc_if_icmpge: return (value1 <= value2 ? labelAddress : pc+1);
		...
		switch(opCode) {
			case RuntimeConstants.opc_if_acmpeq: return (((ReferenceValue)va1).getValue() == ((ReferenceValue)va2).getValue()  ? labelAddress : pc+1);
			case RuntimeConstants.opc_if_acmpne: return (((ReferenceValue)va1).getValue() != ((ReferenceValue)va2).getValue()  ? labelAddress : pc+1);
		}
	}
	return pc;
    }
	...
	case RuntimeConstants.opc_ifnull:
		if (va.getType() != Value.s_reference) {
			System.out.println("Error: '" + RuntimeConstants.opcNames[opCode] + "' requires an reference value on the stack.");
			System.exit(1);
		}
		else {
			return (((ReferenceValue)va).getValue() == null ? labelAddress: pc+1);	// *** come back here, exception probably occurs here...
		}
	...
	while (!done) {
	    ...
	    inst = currentMethod.getInstruction(pc);	// FETCH
	    instructionCount++;
		...
		// YOUR CODE HERE
			case RuntimeConstants.opc_dconst_0:		operandStack.push(new DoubleValue(0.0)); break;
			case RuntimeConstants.opc_dconst_1:		operandStack.push(new DoubleValue(1.0)); break;
			case RuntimeConstants.opc_fconst_0:		operandStack.push(new FloatValue(0)); break;
			case RuntimeConstants.opc_fconst_1:		operandStack.push(new FloatValue(1)); break;
			case RuntimeConstants.opc_fconst_2:		operandStack.push(new FloatValue(2)); break;
	    ...
	    //come back to the 'ldc' instructions and fix them
			case RuntimeConstants.opc_ldc:
				if (((Ldc)inst).getValue() instanceof DoubleValue) {
					operandStack.push(((LdcDouble)inst).getValue());
				}
				else if (((Ldc)inst).getValue() instanceof IntegerValue) {
					operandStack.push(((LdcInteger)inst).getValue());
				}
				else if (((Ldc)inst).getValue() instanceof FloatValue) {
					operandStack.push(((LdcFloat)inst).getValue());
				}
				else if (((Ldc)inst).getValue() instanceof LongValue) {
					operandStack.push(((LdcLong)inst).getValue());
				}
				else if (((Ldc)inst).getValue() instanceof StringValue) {
					operandStack.push(((LdcString)inst).getValue());
				}
				break;
			...
			case RuntimeConstants.opc_bipush:
				int bipush_val = ((SimpleInstruction)inst).getOperand();
				operandStack.push(new IntegerValue((int)((byte)bipush_val)));
				break;

			case RuntimeConstants.opc_sipush:
				int sipush_val = ((SimpleInstruction)inst).getOperand();
				operandStack.push(new IntegerValue((int)((short)sipush_val)));
				break;

			// pop and pop2 opcodes
			case RuntimeConstants.opc_pop: operandStack.pop(); break;
			case RuntimeConstants.opc_pop2: operandStack.pop(); break;
		...
		// YOUR CODE HERE
			...
			case RuntimeConstants.opc_iinc:
				if (activation.getVar(((Iinc)inst).getVarNo()).getType() != Value.s_integer) {
					Error.error("must be of integer type only");
				}
				else {
					// increment the value
					int temp = ((IntegerValue)activation.getVar(((Iinc)inst).getVarNo())).getValue();
					temp += ((Iinc)inst).getInc();
					activation.setVar(new IntegerValue(temp), ((Iinc)inst).getVarNo());
				}
				break;

		// BRANCHING
			case RuntimeConstants.opc_ifeq: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_ifne: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_iflt: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			...
			case RuntimeConstants.opc_ifgt: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_ifge: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_icmpeq: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_icmpne: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_icmplt: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			...
			case RuntimeConstants.opc_if_icmpgt: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_icmpge: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_acmpeq: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_if_acmpne: pc = ifcmpBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_ifnull: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_ifnonnull: pc = ifBranch(opCode, currentMethod.getLabelAddress(((Jump)inst).getLabel()), operandStack, pc); continue;
			case RuntimeConstants.opc_goto: pc = currentMethod.getLabelAddress(((Jump)inst).getLabel()); continue;
	    ...
	    // DUPLICATE OPERATION
			case RuntimeConstants.opc_dup: 			dup(opCode, operandStack);
			case RuntimeConstants.opc_dup_x1: 		dup(opCode, operandStack);
			case RuntimeConstants.opc_dup_x2: 		dup(opCode, operandStack);
			case RuntimeConstants.opc_dup2: 		dup(opCode, operandStack);
			case RuntimeConstants.opc_dup2_x1: 		dup(opCode, operandStack);
			case RuntimeConstants.opc_dup2_x2: 		dup(opCode, operandStack);

		// CONVERSION OPERATION
			case RuntimeConstants.opc_i2b: 			two(opCode, Value.s_integer, Value.s_integer, operandStack);
			case RuntimeConstants.opc_i2s: 			two(opCode, Value.s_integer, Value.s_integer, operandStack);
			case RuntimeConstants.opc_i2c: 			two(opCode, Value.s_integer, Value.s_integer, operandStack);
			case RuntimeConstants.opc_i2d: 			two(opCode, Value.s_integer, Value.s_double, operandStack);
			case RuntimeConstants.opc_i2f: 			two(opCode, Value.s_integer, Value.s_float, operandStack);
			case RuntimeConstants.opc_i2l: 			two(opCode, Value.s_integer, Value.s_long, operandStack);
			case RuntimeConstants.opc_d2i: 			two(opCode, Value.s_double, Value.s_integer, operandStack);
			case RuntimeConstants.opc_d2f: 			two(opCode, Value.s_double, Value.s_float, operandStack);
			case RuntimeConstants.opc_d2l: 			two(opCode, Value.s_double, Value.s_long, operandStack);
			case RuntimeConstants.opc_f2i: 			two(opCode, Value.s_float, Value.s_integer, operandStack);
			case RuntimeConstants.opc_f2d: 			two(opCode, Value.s_float, Value.s_double, operandStack);
			case RuntimeConstants.opc_f2l: 			two(opCode, Value.s_float, Value.s_long, operandStack);
			case RuntimeConstants.opc_l2i: 			two(opCode, Value.s_long, Value.s_integer, operandStack);
			case RuntimeConstants.opc_l2d: 			two(opCode, Value.s_long, Value.s_double, operandStack);
			case RuntimeConstants.opc_l2f: 			two(opCode, Value.s_long, Value.s_float, operandStack);

		// LOGIC OPERATION
			case RuntimeConstants.opc_iand: 		logic(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_ior:			logic(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_ixor:			logic(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_land:			logic(opCode, Value.s_long, operandStack);
			case RuntimeConstants.opc_lor:			logic(opCode, Value.s_long, operandStack);
			case RuntimeConstants.opc_lxor:			logic(opCode, Value.s_long, operandStack);

		// COMPARE OPERATION
			case RuntimeConstants.opc_dcmpg: 		cmp(opCode, Value.s_double, operandStack);
			case RuntimeConstants.opc_dcmpl:		cmp(opCode, Value.s_double, operandStack);
			case RuntimeConstants.opc_fcmpg: 		cmp(opCode, Value.s_float, operandStack);
			case RuntimeConstants.opc_fcmpl:		cmp(opCode, Value.s_float, operandStack);
			case RuntimeConstants.opc_lcmp: 		cmp(opCode, Value.s_long, operandStack);

		// SWAP OPERATION
			case RuntimeConstants.opc_swap: 		swap(opCode, operandStack);

		// NEGATE OPERATION
			case RuntimeConstants.opc_ineg: 		negate(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_dneg:			negate(opCode, Value.s_double, operandStack);
			case RuntimeConstants.opc_fneg:			negate(opCode, Value.s_float, operandStack);
			case RuntimeConstants.opc_lneg:			negate(opCode, Value.s_long, operandStack);

		// SHIFT OPERATION
			case RuntimeConstants.opc_ishl: 		shift(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_ishr: 		shift(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_iushr: 		shift(opCode, Value.s_integer, operandStack);
			case RuntimeConstants.opc_lshl: 		shift(opCode, Value.s_long, operandStack);
			case RuntimeConstants.opc_lshr: 		shift(opCode, Value.s_long, operandStack);
			case RuntimeConstants.opc_lushr: 		shift(opCode, Value.s_long, operandStack);
