...
public class Activation {
	...
	public void store(int type, int address, OperandStack operandStack) {
		...
		// pop the top value(address) on the operand stack
		Value obj = operandStack.pop();

		// assertions/error checking
		if (address > maxAddress) {
			Error.error("address is too large");
		}
		...
		switch(type) {
			case Value.s_reference: variables[address] = (ReferenceValue)obj; break;
			case Value.s_double: variables[address] = (DoubleValue)obj; break;
			case Value.s_float: variables[address] = (FloatValue)obj; break;
			case Value.s_integer: variables[address] = (IntegerValue)obj; break;
			case Value.s_long: variables[address] = (LongValue)obj; break;
		}
	...
	public void load(int type, int address, OperandStack operandStack) {
		...
		if (variables[address] == null) {
			Error.error("value stored at this address is null");
		}

		if (variables[address].getType() != type) {
			Error.error("type mismatch");
		}

		switch (type) {
			case Value.s_reference: operandStack.push((ReferenceValue)variables[address]); break;
			case Value.s_double: operandStack.push((DoubleValue)variables[address]); break;
			case Value.s_float: operandStack.push((FloatValue)variables[address]); break;
			case Value.s_integer: operandStack.push((IntegerValue)variables[address]); break;
			case Value.s_long: operandStack.push((LongValue)variables[address]); break;
		}
	}    
}
